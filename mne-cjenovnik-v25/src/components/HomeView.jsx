  ...
  const personalInflation = useMemo(() => {
    ...
    receipts.forEach(r => {
      ...
      r.items?.forEach(item => {
        ...
        // Handle different price field names from Montenegro API
        const price = item.unitPriceAfterVat || item.priceAfterVat || item.price || item.cijena || 0
        ...
        productPrices[item.code].push({
          price,
          date,
          name: item.name || item.productName || item.naziv || item.code
        })
      })
    })
    ...
    Object.entries(productPrices).forEach(([code, prices]) => {
      ...
      if (!oldest.price || !newest.price || oldest.price <= 0) return
      ...
      changes.push({ code, change, name: newest.name || code })
    })

    if (productsCompared < 2 || totalOldPrice === 0) return null

    const overallChange = ((totalNewPrice - totalOldPrice) / totalOldPrice) * 100
    
    // Check for NaN
    if (isNaN(overallChange)) return null
    
    // Get top increases and decreases
    changes.sort((a, b) => b.change - a.change)
    const topIncreases = changes.filter(c => c.change > 0 && !isNaN(c.change)).slice(0, 3)
    const topDecreases = changes.filter(c => c.change < 0 && !isNaN(c.change)).slice(0, 3)
...
function PriceChangeCard({ change, onClick }) {
  // Calculate direction from prices if not provided
  const oldPrice = change.oldPrice ?? change.previousPrice ?? 0
  const newPrice = change.newPrice ?? change.currentPrice ?? 0
  const isUp = change.direction === 'up' || (newPrice > oldPrice)
  const changePercent = change.changePercent ?? parseFloat(change.percentChange) ?? 0
